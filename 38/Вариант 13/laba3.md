...
# Лабораторная работа 3 "Линейные списки"
# Токмаков А.Ю.
# ИУ10-38
...
__Линейный список__ — это структура данных, представляющая собой конечный, упорядоченный набор элементов, называемых узлами (нодами). Порядок элементов определяется не их физическим расположением в памяти, а explicit ссылочными связями между ними.                                                                            
Основное __преимущество__ списков перед массивами — динамичность: размер списка может легко изменяться во время выполнения программы без необходимости выделения непрерывной области памяти и копирования данных.
...
__Индивидуальное задание__:  Пусть имеется список действительных чисел $a_1 → a_2 → \ldots  → a_n$. Сформировать новый список $b_1 → b_2 → \ldots → b_n$ такой же размерности по следующему правилу: элемент $b_k$ равен сумме элементов исходного списка с номерами от 1 до k.
...
class SingleLinkedList_v7(SingleLinkedList_v6):
    ...
    def new_sum_list(self, list_a) -> Any:
        '''Сформировать новый список по сумме элементов '''
        current_node=list_a._head
        sum_value =0
        current_node_b = self._head
        while current_node is not None:
            sum_value = sum_value + current_node.data
            new_node_b=Node(sum_value,None)
            if current_node_b is None:
                self._head = new_node_b
            else:
                current_node_b.next = new_node_b
...
class DoubleLinkedList_v1:
    '''Реализация АТД Двусвязный линейный список  (DoubleLinkedList_v1) '''
    def __init__(self) -> Self:
        '''Возвращает пустой список'''
        self._head = None
        self._tail = None
    ...
    def __str__(self):
        ...
        while node:
            l.append(str(node.data))
            node = node.next
        return 'DoubleLinked.head -> ' + ' <=> '.join(l) + ' -> None'
        
    def fill_random_list(self, n) -> Any:
        '''создать список из n элементов, заполненных случайными целыми числами'''
        self._head = None
        self._tail = None
        if n>0:
...
print(list7)
print ("-------------------------------- ")
print ("ИНДИВИДУАЛЬНОЕ ЗАДАНИЕ (создать список, k-й элемент нового списка равен сумме элементов исходного списка с номерами от 1 до k)")
list_a=SingleLinkedList_v7()
list_a.fill_sequent_list(10)
print("Последовательный список А:")
print(list_a)
list_b=SingleLinkedList_v7()
list_b.new_sum_list(list_a)
print("Cписок B (k-й элемент нового списка равен сумме элементов исходного списка с номерами от 1 до k):")
print(list_b)

print ("-------------------------------- ")
print ("ДВУНАПРАВЛЕННЫЙ СПИСОК")
d_list=DoubleLinkedList_v1()
d_list.fill_sequent_list(10)
print("Список:" , d_list)
d_list.insert_first_node(0)
print ("Добавить элемент 0 в начало списка: ", d_list )
d_list.insert_last_node(11)
print ("Добавить элемент 11 в конец списка: ", d_list )
d_list.remove_first_node()
print ("Удалить первый элемент: ", d_list )
d_list.remove_last_node()
print ("Удалить последний элемент: ", d_list )
```
